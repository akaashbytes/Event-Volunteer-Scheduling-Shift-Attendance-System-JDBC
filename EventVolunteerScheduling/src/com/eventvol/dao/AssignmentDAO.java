...
public class AssignmentDAO {
    ...
    public List<Assignment> findOverlappingAssignments(
        ...
        try {
            Connection con = DBUtil.getDBConnection();
            String sql =
                "SELECT a.* FROM ASSIGNMENT_TBL a " +"JOIN SHIFT_TBL s ON a.shift_id = s.shift_id " +"WHERE a.volunteer_id = ? " + "AND s.shift_date = ? " +"AND s.start_time < ? " +"AND s.end_time > ?";
            PreparedStatement ps = con.prepareStatement(sql);
    ...
    public List<Assignment> findActiveAssignmentsForVolunteer(
        ...
        try {
            Connection con = DBUtil.getDBConnection();
            String sql =
                "SELECT * FROM ASSIGNMENT_TBL " + "WHERE volunteer_id = ? AND attendance_status = 'PENDING'";
            PreparedStatement ps = con.prepareStatement(sql);
            ps.setString(1, volunteerID);
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
    ...
    public List<Assignment> findActiveAssignmentsForShift(int shiftID) {
        ...
        try {
            Connection con = DBUtil.getDBConnection();
            String sql ="SELECT * FROM ASSIGNMENT_TBL " +"WHERE shift_id = ? AND attendance_status = 'PENDING'";
            PreparedStatement ps = con.prepareStatement(sql);
            ps.setInt(1, shiftID);
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
    ...
    public boolean updateAttendance(
        ...
        try {
            Connection con = DBUtil.getDBConnection();
            String sql ="UPDATE ASSIGNMENT_TBL " +"SET attendance_status = ?, " +"checkin_time = ?, " +"checkout_time = ? " +"WHERE assignment_id = ?";
            PreparedStatement ps = con.prepareStatement(sql);
